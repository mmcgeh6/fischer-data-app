...
# Page configuration
st.set_page_config(
    page_title="Fischer Data Processing - V9",
    page_icon="📊",
    layout="wide"
)
    ...
    if logo_path.exists():
        ...
        st.markdown(
            ...
            <div style="display: flex; align-items: center; padding: 10px 0; margin-bottom: 20px;">
                ...
                <h1 style="color: #24b3aa; margin: 0; font-size: 2.5rem;">Fischer Data Processor V9</h1>
            </div>
            """,
            unsafe_allow_html=True
        )
    else:
        st.title("🔧 Fischer Data Processor V9")
    ...
    for tab_name, tab_config in file_config['tabs'].items():
        try:
            ...
            for col_idx in selected_indices:
                ...
                # Extract and convert value column to numeric (fixes PyArrow errors)
                value_series = pd.to_numeric(df.iloc[:, col_idx], errors='coerce')
...
def main():
    ...
    st.markdown("**Enhanced Flagging** • **Multi-Tab Excel Support** • **File Archiving** • **AI Analysis**")
    st.markdown("---")
    ...
    if uploaded_files:
        ...
        if st.session_state.file_configs:
            ...
            for idx, (file_name, file_path) in enumerate(st.session_state.uploaded_files.items()):
                ...
                if file_type == 'excel_multi_tab':
                    st.info(f"📑 **Multi-Tab Excel File** - {len(config['tabs'])} tabs detected")

                    # For each tab, show column selection
                    for tab_name, tab_config in config['tabs'].items():
                        with st.expander(f"📋 Tab: **{tab_name}**", expanded=True):
                            ...
                            date_column = st.selectbox(
                                ...
                                index=date_col_idx,
                                key=f"date_{file_name}_{tab_name}"
                            )
                            ...
                            for col_idx, col_name in zip(available_cols, column_names):
                                ...
                                if st.checkbox(
                                    ...
                                    value=is_checked,
                                    key=f"col_{file_name}_{tab_name}_{col_idx}"
                                ):
                                    new_selected.append(col_idx)

                            # Update selected columns in config
                            tab_config['date_column'] = date_column
                            tab_config['selected_columns'] = new_selected

                            # Show preview if columns selected
                            if new_selected:
                                st.success(f"✅ {len(new_selected)} column(s) selected from {tab_name}")

                                # Preview data from this tab
                                try:
                                    df_tab = pd.read_excel(file_path, sheet_name=tab_name, header=tab_config['start_row'], nrows=5)
                                    st.caption(f"Preview (first 5 rows from {tab_name}):")

                                    preview_cols = [df_tab.columns[date_column]] + [df_tab.columns[i] for i in new_selected if i < len(df_tab.columns)]
                                    st.dataframe(prepare_df_for_display(df_tab[preview_cols]), height=200)
                                except Exception as e:
                                    st.caption(f"Preview unavailable: {str(e)}")
                            else:
                                st.warning(f"⚠️ No columns selected from {tab_name} - this tab will be skipped")
