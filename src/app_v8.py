"""
Fischer Energy Partners - Data Processing Application V8
Excel Output with Visual Quality Indicators
- All V7 features (per-sensor matching, timestamp normalization)
- Raw merged CSV download
- 15-minute interval resampling with per-sensor matching
- Excel output with color-coded inexact matches
- Consolidated stale data flags (single column + sensor list)
"""
...
# Page configuration
st.set_page_config(
    page_title="Fischer Data Processing - V8",
    page_icon="📊",
    layout="wide"
)
...
def resample_to_quarter_hour(combined_df, tolerance_minutes=2):
    ...
    - Tracks inexact cells for Excel color-coding (no separate flag columns)
...
def export_to_excel(resampled_df, inexact_cells, output_path):
    """
    Export resampled data to Excel with color-coded inexact matches.
    ...
    try:
        ...
        sensor_cols = [col for col in resampled_df.columns
                      if col not in ['Date', 'Stale_Data_Flag', 'Stale_Sensors']]
...
def main():
    ...
    # Header
    st.title("📊 Fischer Data Processing - V8")
    st.markdown("**Excel Output with Color-Coded Quality** • Per-Sensor Matching • AI Analysis • Timestamp Normalization")
    st.markdown("---")
